...
const Inspections = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const loadedInspections: Inspection[] = inspectionResponse.data;
      const loadedRepeatabilityRecords: RepeatabilityRecord[] =
        repeatabilityResponse.data;
      const loadedEccentricityRecords: EccentricityRecord[] =
        eccentricityResponse.data;

      setInstruments(instrumentResponse.data);
      setInspections(loadedInspections);
      setTestRecords(testResponse.data);
      setRepeatabilityRecords(loadedRepeatabilityRecords);
      setEccentricityRecords(loadedEccentricityRecords);

      const repeatabilityResultEntries = await Promise.all(
        loadedInspections.map(async (inspection) => {
          const records = loadedRepeatabilityRecords.filter(
            (record) => record.inspectionId === inspection.id
          );

          const latestRunId = records
            .map((record) => record.testRunId)
            .filter((id): id is number => id !== undefined)
            .sort((a, b) => b - a)[0];

          if (latestRunId === undefined) {
            return [inspection.id, "PENDING"] as const;
          }
      ...
      const eccentricityResultEntries = await Promise.all(
        loadedInspections.map(async (inspection) => {
          const records = loadedEccentricityRecords.filter(
            (record) => record.inspectionId === inspection.id
          );
      ...
      setRepeatabilityResults(
        Object.fromEntries(repeatabilityResultEntries)
      );
      setEccentricityResults(
        Object.fromEntries(eccentricityResultEntries)
      );
    } catch (err) {
  ...
  const filteredInstruments = useMemo(() => {
    ...
    return instruments.filter(
      (instrument) =>
        instrument.serialNumber.toLowerCase().includes(value) ||
        instrument.model.toLowerCase().includes(value) ||
        instrument.manufacturer.toLowerCase().includes(value)
    );
  }, [instruments, search]);
  ...
  const getInspectionTests = (inspectionId: number) => {
    return testRecords.filter(
      (record) => record.inspectionId === inspectionId
    );
  };

  const getInspectionTestStages = (inspectionId: number) => {
    const tests = getInspectionTests(inspectionId);

    const hasWeighingPerformance = tests.some(
      (test) => test.testType === "WEIGHING_PERFORMANCE"
    );

    const hasRepeatability = repeatabilityRecords.some(
      (record) => record.inspectionId === inspectionId
    );

    const hasEccentricity = eccentricityRecords.some(
      (record) => record.inspectionId === inspectionId
    );

    const count =
      Number(hasWeighingPerformance) +
      Number(hasRepeatability) +
      Number(hasEccentricity);
  ...
  const getInspectionStageResults = (inspectionId: number): string[] => {
    const tests = getInspectionTests(inspectionId);

    const weighingResults = tests
      .filter(
        (test) => test.testType === "WEIGHING_PERFORMANCE"
      )
    ...
    const weighingResult =
      weighingResults.length > 0
        ? weighingResults[weighingResults.length - 1]
        : "PENDING";

    const repeatabilityResult =
      repeatabilityResults[inspectionId] || "PENDING";

    const eccentricityResult =
      eccentricityResults[inspectionId] || "PENDING";
  ...
  const getWeather = async (
    ...
    try {
      ...
      setMessage("Environment data fetched successfully.");
    } catch (err) {
  ...
  const assessEnvironment = async (
    ...
    try {
      const response = await api.post("/environment/assess", {
        temperature: temp,
      ...
      setEnvironmentStatus(
        response.data?.overall || "NOT_ASSESSED"
      );

      return response.data;
    } catch (err) {
      console.error("Environment assessment failed", err);
      return null;
    }
  };
  ...
  const resetEnvironment = () => {
    ...
    setEnvironmentStatus("NOT_ASSESSED");
  };

  const saveEnvironmentRecord = async (
    inspectionId: number
  ) => {
    if (
      temperature === null ||
  ...
  const createInspection = async () => {
    ...
    try {
      ...
      await saveEnvironmentRecord(inspectionId);

      setMessage(
        `Inspection #${inspectionId} created successfully for ${selectedInstrument.serialNumber}. Environment data saved.`
      );
  ...
  const getEnvironmentClass = (status: string) => {
    if (status === "NORMAL") {
      return "bg-emerald-50 text-emerald-700 border-emerald-200";
    }
  ...
  const getWorkflowStep = (
    ...
    const tests = getInspectionTests(inspection.id);
...
<p className="mt-1 text-sm text-slate-600">
  Serial Number: {selectedInstrument.serialNumber}
                ...
                <p className="mt-1 text-sm text-slate-500">
                  ...
                  {selectedInstrument.model} • Class{" "}
...
<p className="mt-1 text-sm text-slate-500">
  Serial Number:{" "}
  <span className="font-semibold text-slate-700">
    {instrument?.serialNumber || "Unknown"}
  </span>
</p>
                      ...
                      <div className="mt-5 grid gap-3 md:grid-cols-3">
                        {testStages.hasWeighingPerformance && (
                          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                            ...
                            <p className="mt-1 text-xs text-slate-500">
                              {tests.length} measurement record
                              {tests.length === 1 ? "" : "s"}
                            </p>
                          </div>
                        )}
